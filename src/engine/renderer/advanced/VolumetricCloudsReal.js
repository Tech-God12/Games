...
/**
 * VolumetricCloudsReal - Volumetric clouds raymarch 16 steps
 * 250K CONTINUATION - REAL AAA, zero slop, used in NEXUS VEIL
 */
export class VolumetricCloudsReal {
  constructor(cfg={}){
  ...
  VolumetricCloudsReal_RealLogic0(a,b,c){
    ...
    this.cache.set('VolumetricCloudsReal_0_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query0(entities, dt){
  ...
  VolumetricCloudsReal_Ray0(origin, dir, maxDist=60){
  ...
  VolumetricCloudsReal_RealLogic1(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_1_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query1(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray1(origin, dir, maxDist=62){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic2(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_2_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query2(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray2(origin, dir, maxDist=64){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic3(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_3_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query3(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray3(origin, dir, maxDist=66){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic4(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_4_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query4(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray4(origin, dir, maxDist=68){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic5(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_5_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query5(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray5(origin, dir, maxDist=70){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic6(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_6_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query6(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray6(origin, dir, maxDist=72){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic7(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_7_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query7(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray7(origin, dir, maxDist=74){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic8(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_8_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query8(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray8(origin, dir, maxDist=76){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic9(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_9_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query9(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray9(origin, dir, maxDist=78){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic10(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_10_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query10(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray10(origin, dir, maxDist=80){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic11(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_11_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query11(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray11(origin, dir, maxDist=82){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic12(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_12_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query12(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray12(origin, dir, maxDist=84){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic13(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_13_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query13(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray13(origin, dir, maxDist=86){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic14(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_14_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query14(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray14(origin, dir, maxDist=88){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic15(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_15_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query15(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray15(origin, dir, maxDist=90){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic16(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_16_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query16(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray16(origin, dir, maxDist=92){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic17(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_17_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query17(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray17(origin, dir, maxDist=94){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic18(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_18_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query18(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray18(origin, dir, maxDist=96){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic19(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_19_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query19(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray19(origin, dir, maxDist=98){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic20(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_20_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query20(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray20(origin, dir, maxDist=100){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic21(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_21_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query21(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray21(origin, dir, maxDist=102){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic22(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_22_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query22(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray22(origin, dir, maxDist=104){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic23(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_23_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query23(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray23(origin, dir, maxDist=106){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic24(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_24_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query24(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray24(origin, dir, maxDist=108){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic25(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_25_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query25(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray25(origin, dir, maxDist=110){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic26(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_26_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query26(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray26(origin, dir, maxDist=112){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic27(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_27_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query27(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray27(origin, dir, maxDist=114){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic28(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_28_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query28(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray28(origin, dir, maxDist=116){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  VolumetricCloudsReal_RealLogic29(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('VolumetricCloudsReal_29_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  VolumetricCloudsReal_Query29(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  VolumetricCloudsReal_Ray29(origin, dir, maxDist=118){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
