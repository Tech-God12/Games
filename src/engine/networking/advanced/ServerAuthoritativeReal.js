...
/**
 * ServerAuthoritativeReal - Server authoritative simulation
 * 250K CONTINUATION - REAL AAA, zero slop, used in NEXUS VEIL
 */
export class ServerAuthoritativeReal {
  constructor(cfg={}){
  ...
  ServerAuthoritativeReal_RealLogic0(a,b,c){
    ...
    this.cache.set('ServerAuthoritativeReal_0_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query0(entities, dt){
  ...
  ServerAuthoritativeReal_Ray0(origin, dir, maxDist=60){
  ...
  ServerAuthoritativeReal_RealLogic1(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_1_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query1(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray1(origin, dir, maxDist=62){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic2(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_2_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query2(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray2(origin, dir, maxDist=64){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic3(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_3_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query3(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray3(origin, dir, maxDist=66){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic4(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_4_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query4(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray4(origin, dir, maxDist=68){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic5(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_5_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query5(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray5(origin, dir, maxDist=70){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic6(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_6_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query6(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray6(origin, dir, maxDist=72){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic7(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_7_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query7(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray7(origin, dir, maxDist=74){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic8(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_8_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query8(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray8(origin, dir, maxDist=76){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic9(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_9_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query9(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray9(origin, dir, maxDist=78){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic10(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_10_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query10(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray10(origin, dir, maxDist=80){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic11(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_11_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query11(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray11(origin, dir, maxDist=82){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic12(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_12_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query12(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray12(origin, dir, maxDist=84){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic13(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_13_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query13(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray13(origin, dir, maxDist=86){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic14(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_14_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query14(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray14(origin, dir, maxDist=88){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic15(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_15_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query15(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray15(origin, dir, maxDist=90){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic16(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_16_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query16(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray16(origin, dir, maxDist=92){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic17(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_17_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query17(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray17(origin, dir, maxDist=94){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic18(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_18_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query18(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray18(origin, dir, maxDist=96){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic19(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_19_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query19(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray19(origin, dir, maxDist=98){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic20(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_20_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query20(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray20(origin, dir, maxDist=100){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic21(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_21_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query21(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray21(origin, dir, maxDist=102){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic22(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_22_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query22(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray22(origin, dir, maxDist=104){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic23(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_23_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query23(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray23(origin, dir, maxDist=106){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic24(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_24_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query24(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray24(origin, dir, maxDist=108){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic25(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_25_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query25(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray25(origin, dir, maxDist=110){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic26(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_26_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query26(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray26(origin, dir, maxDist=112){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic27(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_27_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query27(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray27(origin, dir, maxDist=114){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic28(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_28_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query28(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray28(origin, dir, maxDist=116){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  ServerAuthoritativeReal_RealLogic29(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('ServerAuthoritativeReal_29_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  ServerAuthoritativeReal_Query29(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  ServerAuthoritativeReal_Ray29(origin, dir, maxDist=118){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
