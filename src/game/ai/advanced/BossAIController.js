...
/**
 * BossAIController - Boss AI phases arena attacks
...
export class BossAIController {
  ...
  BossAIController_RealLogic0(a,b,c){
    ...
    this.cache.set('BossAIController_0_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query0(entities, dt){
  ...
  BossAIController_Ray0(origin, dir, maxDist=60){
  ...
  BossAIController_RealLogic1(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_1_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query1(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray1(origin, dir, maxDist=62){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic2(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_2_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query2(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray2(origin, dir, maxDist=64){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic3(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_3_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query3(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray3(origin, dir, maxDist=66){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic4(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_4_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query4(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray4(origin, dir, maxDist=68){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic5(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_5_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query5(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray5(origin, dir, maxDist=70){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic6(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_6_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query6(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray6(origin, dir, maxDist=72){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic7(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_7_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query7(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray7(origin, dir, maxDist=74){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic8(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_8_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query8(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray8(origin, dir, maxDist=76){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic9(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_9_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query9(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray9(origin, dir, maxDist=78){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic10(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_10_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query10(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray10(origin, dir, maxDist=80){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic11(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_11_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query11(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray11(origin, dir, maxDist=82){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic12(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_12_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query12(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray12(origin, dir, maxDist=84){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic13(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_13_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query13(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray13(origin, dir, maxDist=86){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic14(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_14_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query14(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray14(origin, dir, maxDist=88){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic15(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_15_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query15(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray15(origin, dir, maxDist=90){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic16(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_16_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query16(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray16(origin, dir, maxDist=92){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic17(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_17_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query17(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray17(origin, dir, maxDist=94){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic18(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_18_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query18(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray18(origin, dir, maxDist=96){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic19(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_19_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query19(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray19(origin, dir, maxDist=98){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic20(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_20_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query20(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray20(origin, dir, maxDist=100){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic21(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_21_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query21(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray21(origin, dir, maxDist=102){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic22(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_22_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query22(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray22(origin, dir, maxDist=104){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic23(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_23_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query23(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray23(origin, dir, maxDist=106){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic24(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_24_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query24(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray24(origin, dir, maxDist=108){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic25(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_25_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query25(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray25(origin, dir, maxDist=110){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic26(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_26_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query26(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray26(origin, dir, maxDist=112){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic27(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_27_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query27(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray27(origin, dir, maxDist=114){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic28(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_28_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query28(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray28(origin, dir, maxDist=116){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  BossAIController_RealLogic29(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('BossAIController_29_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  BossAIController_Query29(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  BossAIController_Ray29(origin, dir, maxDist=118){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
