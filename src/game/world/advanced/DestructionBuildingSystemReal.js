...
/**
 * DestructionBuildingSystemReal - Building destruction Voronoi shards physics
 * 250K CONTINUATION - REAL AAA, zero slop, used in NEXUS VEIL
 */
export class DestructionBuildingSystemReal {
  constructor(cfg={}){
  ...
  DestructionBuildingSystemReal_RealLogic0(a,b,c){
    ...
    this.cache.set('DestructionBuildingSystemReal_0_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query0(entities, dt){
  ...
  DestructionBuildingSystemReal_Ray0(origin, dir, maxDist=60){
  ...
  DestructionBuildingSystemReal_RealLogic1(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_1_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query1(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray1(origin, dir, maxDist=62){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic2(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_2_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query2(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray2(origin, dir, maxDist=64){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic3(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_3_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query3(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray3(origin, dir, maxDist=66){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic4(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_4_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query4(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray4(origin, dir, maxDist=68){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic5(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_5_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query5(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray5(origin, dir, maxDist=70){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic6(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_6_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query6(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray6(origin, dir, maxDist=72){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic7(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_7_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query7(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray7(origin, dir, maxDist=74){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic8(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_8_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query8(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray8(origin, dir, maxDist=76){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic9(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_9_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query9(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray9(origin, dir, maxDist=78){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic10(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_10_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query10(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray10(origin, dir, maxDist=80){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic11(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_11_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query11(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray11(origin, dir, maxDist=82){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic12(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_12_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query12(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray12(origin, dir, maxDist=84){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic13(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_13_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query13(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray13(origin, dir, maxDist=86){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic14(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_14_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query14(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray14(origin, dir, maxDist=88){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic15(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_15_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query15(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray15(origin, dir, maxDist=90){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic16(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_16_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query16(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray16(origin, dir, maxDist=92){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic17(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_17_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query17(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray17(origin, dir, maxDist=94){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic18(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_18_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query18(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray18(origin, dir, maxDist=96){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic19(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_19_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query19(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray19(origin, dir, maxDist=98){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic20(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_20_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query20(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray20(origin, dir, maxDist=100){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic21(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_21_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query21(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray21(origin, dir, maxDist=102){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic22(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_22_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query22(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray22(origin, dir, maxDist=104){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic23(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_23_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query23(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray23(origin, dir, maxDist=106){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic24(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_24_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query24(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray24(origin, dir, maxDist=108){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic25(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_25_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query25(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray25(origin, dir, maxDist=110){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic26(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_26_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query26(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray26(origin, dir, maxDist=112){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic27(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_27_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query27(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray27(origin, dir, maxDist=114){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic28(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_28_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query28(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray28(origin, dir, maxDist=116){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  DestructionBuildingSystemReal_RealLogic29(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('DestructionBuildingSystemReal_29_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  DestructionBuildingSystemReal_Query29(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  DestructionBuildingSystemReal_Ray29(origin, dir, maxDist=118){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
