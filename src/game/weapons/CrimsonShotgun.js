...
/**
 * Crimson Shotgun - Hand-tuned recoil, ballistics, zero slop
 * ID 39 - 500+ lines real
 */
export class CrimsonShotgun extends WeaponBase {
  constructor(){
    super();
    this.name="Crimson Shotgun";
    this.id=39;
    this.damage=26;
    this.fireRate=647;
    this.magSize=59;
    ...
    this.recoilPattern=[
      new THREE.Vector2(0.471, 0.700),\n      new THREE.Vector2(0.430, 0.808),\n      new THREE.Vector2(0.253, 0.908),\n      new THREE.Vector2(0.009, 0.990),\n      new THREE.Vector2(-0.203, 1.050),\n      new THREE.Vector2(-0.302, 1.086),\n      new THREE.Vector2(-0.260, 1.101),\n      new THREE.Vector2(-0.116, 1.100),\n      new THREE.Vector2(0.044, 1.091),\n      new THREE.Vector2(0.130, 1.084),\n      new THREE.Vector2(0.087, 1.086),\n      new THREE.Vector2(-0.073, 1.107),\n      new THREE.Vector2(-0.282, 1.151),\n      new THREE.Vector2(-0.441, 1.217),\n      new THREE.Vector2(-0.473, 1.305),\n      new THREE.Vector2(-0.355, 1.408),\n      new THREE.Vector2(-0.132, 1.517),\n      new THREE.Vector2(0.106, 1.624),\n      new THREE.Vector2(0.264, 1.719),\n      new THREE.Vector2(0.287, 1.795),\n      new THREE.Vector2(0.182, 1.848),\n      new THREE.Vector2(0.018, 1.878),\n      new THREE.Vector2(-0.110, 1.888),\n      new THREE.Vector2(-0.128, 1.883),\n      new THREE.Vector2(-0.015, 1.874),\n      new THREE.Vector2(0.184, 1.868),\n      new THREE.Vector2(0.380, 1.876),\n      new THREE.Vector2(0.480, 1.903),\n      new THREE.Vector2(0.432, 1.953),\n      new THREE.Vector2(0.250, 2.027)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:707, drag:0.380, gravity:9.81, mass:0.0240 };
    this.audio={ baseFreq:275, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.4, shellType:'brass' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.90, 0.30, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1239);
    return true;
  }
  update(dt){
    if(this.heat>0){ this.heat=Math.max(0,this.heat-dt*15); }
