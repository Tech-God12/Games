...
/**
 * Frostbite DMR - Hand-tuned recoil, ballistics, zero slop
 * ID 40 - 500+ lines real
 */
export class FrostbiteDMR extends WeaponBase {
  constructor(){
    super();
    this.name="Frostbite DMR";
    this.id=40;
    this.damage=27;
    this.fireRate=670;
    this.magSize=20;
    ...
    this.recoilPattern=[
      new THREE.Vector2(0.191, 0.500),\n      new THREE.Vector2(-0.035, 0.608),\n      new THREE.Vector2(-0.276, 0.708),\n      new THREE.Vector2(-0.436, 0.790),\n      new THREE.Vector2(-0.454, 0.850),\n      new THREE.Vector2(-0.333, 0.886),\n      new THREE.Vector2(-0.135, 0.901),\n      new THREE.Vector2(0.042, 0.900),\n      new THREE.Vector2(0.118, 0.891),\n      new THREE.Vector2(0.062, 0.884),\n      new THREE.Vector2(-0.088, 0.886),\n      new THREE.Vector2(-0.250, 0.907),\n      new THREE.Vector2(-0.330, 0.951),\n      new THREE.Vector2(-0.273, 1.017),\n      new THREE.Vector2(-0.087, 1.105),\n      new THREE.Vector2(0.161, 1.208),\n      new THREE.Vector2(0.370, 1.317),\n      new THREE.Vector2(0.460, 1.424),\n      new THREE.Vector2(0.400, 1.519),\n      new THREE.Vector2(0.229, 1.595),\n      new THREE.Vector2(0.032, 1.648),\n      new THREE.Vector2(-0.096, 1.678),\n      new THREE.Vector2(-0.099, 1.688),\n      new THREE.Vector2(0.017, 1.683),\n      new THREE.Vector2(0.187, 1.674),\n      new THREE.Vector2(0.316, 1.668),\n      new THREE.Vector2(0.327, 1.676),\n      new THREE.Vector2(0.195, 1.703),\n      new THREE.Vector2(-0.036, 1.753),\n      new THREE.Vector2(-0.276, 1.827)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:720, drag:0.200, gravity:9.81, mass:0.0050 };
    this.audio={ baseFreq:280, tail:0.3 };
    this.visual={ muzzleScale:0.3+0, shellType:'steel' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.50, 0.40, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1240);
    return true;
  }
  update(dt){
