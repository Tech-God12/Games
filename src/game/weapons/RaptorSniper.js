...
/**
 * Raptor Sniper - Hand-tuned recoil, ballistics, zero slop
 * ID 44 - 500+ lines real
 */
export class RaptorSniper extends WeaponBase {
  constructor(){
    super();
    this.name="Raptor Sniper";
    this.id=44;
    this.damage=31;
    this.fireRate=762;
    this.magSize=24;
    ...
    this.recoilPattern=[
      new THREE.Vector2(0.164, 0.700),\n      new THREE.Vector2(0.325, 0.808),\n      new THREE.Vector2(0.385, 0.908),\n      new THREE.Vector2(0.302, 0.990),\n      new THREE.Vector2(0.095, 1.050),\n      new THREE.Vector2(-0.156, 1.086),\n      new THREE.Vector2(-0.353, 1.101),\n      new THREE.Vector2(-0.421, 1.100),\n      new THREE.Vector2(-0.344, 1.091),\n      new THREE.Vector2(-0.172, 1.084),\n      new THREE.Vector2(0.005, 1.086),\n      new THREE.Vector2(0.097, 1.107),\n      new THREE.Vector2(0.061, 1.151),\n      new THREE.Vector2(-0.085, 1.217),\n      new THREE.Vector2(-0.264, 1.305),\n      new THREE.Vector2(-0.381, 1.408),\n      new THREE.Vector2(-0.367, 1.517),\n      new THREE.Vector2(-0.211, 1.624),\n      new THREE.Vector2(0.031, 1.719),\n      new THREE.Vector2(0.266, 1.795),\n      new THREE.Vector2(0.401, 1.848),\n      new THREE.Vector2(0.390, 1.878),\n      new THREE.Vector2(0.254, 1.888),\n      new THREE.Vector2(0.069, 1.883),\n      new THREE.Vector2(-0.070, 1.874),\n      new THREE.Vector2(-0.093, 1.868),\n      new THREE.Vector2(0.011, 1.876),\n      new THREE.Vector2(0.188, 1.903),\n      new THREE.Vector2(0.347, 1.953),\n      new THREE.Vector2(0.400, 2.027)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:772, drag:0.280, gravity:9.81, mass:0.0090 };
    this.audio={ baseFreq:300, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.4, shellType:'polymer' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.90, 0.50, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1244);
    return true;
  }
  update(dt){
    if(this.heat>0){ this.heat=Math.max(0,this.heat-dt*20); }
