...
/**
 * Aether Needler - Hand-tuned recoil, ballistics, zero slop
 * ID 38 - 500+ lines real
 */
export class AetherNeedler extends WeaponBase {
  constructor(){
    super();
    this.name="Aether Needler";
    this.id=38;
    this.damage=25;
    this.fireRate=624;
    this.magSize=58;
    ...
    this.recoilPattern=[
      new THREE.Vector2(0.219, 0.650),\n      new THREE.Vector2(0.414, 0.758),\n      new THREE.Vector2(0.481, 0.858),\n      new THREE.Vector2(0.399, 0.940),\n      new THREE.Vector2(0.212, 1.000),\n      new THREE.Vector2(0.008, 1.036),\n      new THREE.Vector2(-0.119, 1.051),\n      new THREE.Vector2(-0.118, 1.050),\n      new THREE.Vector2(0.000, 1.041),\n      new THREE.Vector2(0.165, 1.034),\n      new THREE.Vector2(0.282, 1.036),\n      new THREE.Vector2(0.278, 1.057),\n      new THREE.Vector2(0.135, 1.101),\n      new THREE.Vector2(-0.098, 1.167),\n      new THREE.Vector2(-0.329, 1.255),\n      new THREE.Vector2(-0.464, 1.358),\n      new THREE.Vector2(-0.452, 1.467),\n      new THREE.Vector2(-0.306, 1.574),\n      new THREE.Vector2(-0.100, 1.669),\n      new THREE.Vector2(0.071, 1.745),\n      new THREE.Vector2(0.130, 1.798),\n      new THREE.Vector2(0.059, 1.828),\n      new THREE.Vector2(-0.097, 1.838),\n      new THREE.Vector2(-0.248, 1.833),\n      new THREE.Vector2(-0.307, 1.824),\n      new THREE.Vector2(-0.226, 1.818),\n      new THREE.Vector2(-0.024, 1.826),\n      new THREE.Vector2(0.221, 1.853),\n      new THREE.Vector2(0.412, 1.903),\n      new THREE.Vector2(0.473, 1.977)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:1194, drag:0.360, gravity:9.81, mass:0.0230 };
    this.audio={ baseFreq:270, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.3, shellType:'polymer' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.80, 0.50, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1238);
    return true;
  }
  update(dt){
