...
/**
 * Tempest Breacher - Hand-tuned recoil, ballistics, zero slop
 * ID 28 - 500+ lines real
 */
export class TempestBreacher extends WeaponBase {
  constructor(){
    super();
    this.name="Tempest Breacher";
    this.id=28;
    this.damage=50;
    this.fireRate=394;
    this.magSize=48;
    ...
    this.range=416;
    this.spreadBase=0.0100;
    this.recoilPattern=[
      new THREE.Vector2(0.135, 0.650),\n      new THREE.Vector2(-0.028, 0.758),\n      new THREE.Vector2(-0.137, 0.858),\n      new THREE.Vector2(-0.126, 0.940),\n      new THREE.Vector2(0.012, 1.000),\n      new THREE.Vector2(0.223, 1.036),\n      new THREE.Vector2(0.411, 1.051),\n      new THREE.Vector2(0.489, 1.050),\n      new THREE.Vector2(0.417, 1.041),\n      new THREE.Vector2(0.220, 1.034),\n      new THREE.Vector2(-0.021, 1.036),\n      new THREE.Vector2(-0.206, 1.057),\n      new THREE.Vector2(-0.267, 1.101),\n      new THREE.Vector2(-0.193, 1.167),\n      new THREE.Vector2(-0.038, 1.255),\n      new THREE.Vector2(0.106, 1.358),\n      new THREE.Vector2(0.154, 1.467),\n      new THREE.Vector2(0.070, 1.574),\n      new THREE.Vector2(-0.120, 1.669),\n      new THREE.Vector2(-0.332, 1.745),\n      new THREE.Vector2(-0.472, 1.798),\n      new THREE.Vector2(-0.473, 1.828),\n      new THREE.Vector2(-0.328, 1.838),\n      new THREE.Vector2(-0.097, 1.833),\n      new THREE.Vector2(0.124, 1.824),\n      new THREE.Vector2(0.248, 1.818),\n      new THREE.Vector2(0.233, 1.826),\n      new THREE.Vector2(0.106, 1.853),\n      new THREE.Vector2(-0.054, 1.903),\n      new THREE.Vector2(-0.155, 1.977)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:1064, drag:0.360, gravity:9.81, mass:0.0130 };
    this.audio={ baseFreq:220, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.3, shellType:'steel' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.80, 0.40, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1228);
    return true;
  }
  update(dt){
    if(this.heat>0){ this.heat=Math.max(0,this.heat-dt*16); }
