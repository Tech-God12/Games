...
/**
 * Thunderclap LMG - Hand-tuned recoil, ballistics, zero slop
 * ID 41 - 500+ lines real
 */
export class ThunderclapLMG extends WeaponBase {
  constructor(){
    super();
    this.name="Thunderclap LMG";
    this.id=41;
    this.damage=28;
    this.fireRate=693;
    this.magSize=21;
    ...
    this.range=172;
    this.spreadBase=0.0030;
    this.recoilPattern=[
      new THREE.Vector2(-0.246, 0.550),\n      new THREE.Vector2(-0.426, 0.658),\n      new THREE.Vector2(-0.488, 0.758),\n      new THREE.Vector2(-0.399, 0.840),\n      new THREE.Vector2(-0.192, 0.900),\n      new THREE.Vector2(0.048, 0.936),\n      new THREE.Vector2(0.223, 0.951),\n      new THREE.Vector2(0.269, 0.950),\n      new THREE.Vector2(0.182, 0.941),\n      new THREE.Vector2(0.024, 0.934),\n      new THREE.Vector2(-0.114, 0.936),\n      new THREE.Vector2(-0.149, 0.957),\n      new THREE.Vector2(-0.052, 1.001),\n      new THREE.Vector2(0.144, 1.067),\n      new THREE.Vector2(0.353, 1.155),\n      new THREE.Vector2(0.479, 1.258),\n      new THREE.Vector2(0.462, 1.367),\n      new THREE.Vector2(0.304, 1.474),\n      new THREE.Vector2(0.068, 1.569),\n      new THREE.Vector2(-0.147, 1.645),\n      new THREE.Vector2(-0.256, 1.698),\n      new THREE.Vector2(-0.227, 1.728),\n      new THREE.Vector2(-0.092, 1.738),\n      new THREE.Vector2(0.066, 1.733),\n      new THREE.Vector2(0.156, 1.724),\n      new THREE.Vector2(0.118, 1.718),\n      new THREE.Vector2(-0.042, 1.726),\n      new THREE.Vector2(-0.259, 1.753),\n      new THREE.Vector2(-0.437, 1.803),\n      new THREE.Vector2(-0.492, 1.877)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:733, drag:0.220, gravity:9.81, mass:0.0060 };
    this.audio={ baseFreq:285, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.1, shellType:'polymer' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.60, 0.50, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1241);
    return true;
  }
  update(dt){
    if(this.heat>0){ this.heat=Math.max(0,this.heat-dt*17); }
