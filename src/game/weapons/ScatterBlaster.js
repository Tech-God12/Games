...
/**
 * Scatter Blaster - Hand-tuned recoil, ballistics, zero slop
 * ID 27 - 500+ lines real
 */
export class ScatterBlaster extends WeaponBase {
  constructor(){
    super();
    this.name="Scatter Blaster";
    this.id=27;
    this.damage=49;
    this.fireRate=371;
    this.magSize=47;
    ...
    this.recoilPattern=[
      new THREE.Vector2(0.116, 0.600),\n      new THREE.Vector2(0.020, 0.708),\n      new THREE.Vector2(-0.145, 0.808),\n      new THREE.Vector2(-0.287, 0.890),\n      new THREE.Vector2(-0.321, 0.950),\n      new THREE.Vector2(-0.214, 0.986),\n      new THREE.Vector2(0.005, 1.001),\n      new THREE.Vector2(0.250, 1.000),\n      new THREE.Vector2(0.423, 0.991),\n      new THREE.Vector2(0.458, 0.984),\n      new THREE.Vector2(0.350, 0.986),\n      new THREE.Vector2(0.157, 1.007),\n      new THREE.Vector2(-0.026, 1.051),\n      new THREE.Vector2(-0.115, 1.117),\n      new THREE.Vector2(-0.072, 1.205),\n      new THREE.Vector2(0.073, 1.308),\n      new THREE.Vector2(0.238, 1.417),\n      new THREE.Vector2(0.331, 1.524),\n      new THREE.Vector2(0.290, 1.619),\n      new THREE.Vector2(0.115, 1.695),\n      new THREE.Vector2(-0.131, 1.748),\n      new THREE.Vector2(-0.350, 1.778),\n      new THREE.Vector2(-0.455, 1.788),\n      new THREE.Vector2(-0.412, 1.783),\n      new THREE.Vector2(-0.250, 1.774),\n      new THREE.Vector2(-0.052, 1.768),\n      new THREE.Vector2(0.087, 1.776),\n      new THREE.Vector2(0.104, 1.803),\n      new THREE.Vector2(-0.003, 1.853),\n      new THREE.Vector2(-0.172, 1.927)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:1051, drag:0.340, gravity:9.81, mass:0.0120 };
    this.audio={ baseFreq:215, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.2, shellType:'brass' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.70, 0.30, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1227);
    return true;
  }
  update(dt){
