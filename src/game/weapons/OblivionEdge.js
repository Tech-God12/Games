...
/**
 * Oblivion Edge - Hand-tuned recoil, ballistics, zero slop
 * ID 31 - 500+ lines real
 */
export class OblivionEdge extends WeaponBase {
  constructor(){
    super();
    this.name="Oblivion Edge";
    this.id=31;
    this.damage=53;
    this.fireRate=463;
    this.magSize=51;
    ...
    this.recoilPattern=[
      new THREE.Vector2(-0.293, 0.550),\n      new THREE.Vector2(-0.106, 0.658),\n      new THREE.Vector2(0.050, 0.758),\n      new THREE.Vector2(0.100, 0.840),\n      new THREE.Vector2(0.020, 0.900),\n      new THREE.Vector2(-0.147, 0.936),\n      new THREE.Vector2(-0.313, 0.951),\n      new THREE.Vector2(-0.388, 0.950),\n      new THREE.Vector2(-0.321, 0.941),\n      new THREE.Vector2(-0.125, 0.934),\n      new THREE.Vector2(0.126, 0.936),\n      new THREE.Vector2(0.334, 0.957),\n      new THREE.Vector2(0.418, 1.001),\n      new THREE.Vector2(0.356, 1.067),\n      new THREE.Vector2(0.191, 1.155),\n      new THREE.Vector2(0.011, 1.258),\n      new THREE.Vector2(-0.093, 1.367),\n      new THREE.Vector2(-0.070, 1.474),\n      new THREE.Vector2(0.067, 1.569),\n      new THREE.Vector2(0.248, 1.645),\n      new THREE.Vector2(0.377, 1.698),\n      new THREE.Vector2(0.379, 1.728),\n      new THREE.Vector2(0.237, 1.738),\n      new THREE.Vector2(-0.000, 1.733),\n      new THREE.Vector2(-0.240, 1.724),\n      new THREE.Vector2(-0.390, 1.718),\n      new THREE.Vector2(-0.395, 1.726),\n      new THREE.Vector2(-0.271, 1.753),\n      new THREE.Vector2(-0.087, 1.803),\n      new THREE.Vector2(0.060, 1.877)
    ];
    this.recoilIndex=0;
    this.bloom=0;
    this.heat=0;
    ...
    this.ballistics={ muzzleVelocity:1103, drag:0.220, gravity:9.81, mass:0.0160 };
    this.audio={ baseFreq:235, tail:0.3 };
    this.visual={ muzzleScale:0.3+0.1, shellType:'steel' };
  ...
  muzzleEffect(origin, dir){
    return {
      ...
      shell:{ pos:origin.clone().add(new THREE.Vector3(0.2,-0.1,0.3)), vel:new THREE.Vector3(0.60, 0.40, (Math.random()-0.5)), type:this.visual.shellType }
    };
  }
  reload(){
    ...
    setTimeout(()=>{ this.ammo+=take; this.reserve-=take; this.state.reloading=false; this.recoilIndex=0; this.bloom=0; }, 1231);
    return true;
  }
  update(dt){
    if(this.heat>0){ this.heat=Math.max(0,this.heat-dt*19); }
