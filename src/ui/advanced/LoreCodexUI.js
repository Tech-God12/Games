...
/**
 * LoreCodexUI - Lore codex collectibles
 * 250K CONTINUATION - REAL AAA, zero slop, used in NEXUS VEIL
 */
export class LoreCodexUI {
  constructor(cfg={}){
  ...
  LoreCodexUI_RealLogic0(a,b,c){
    ...
    this.cache.set('LoreCodexUI_0_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query0(entities, dt){
  ...
  LoreCodexUI_Ray0(origin, dir, maxDist=60){
  ...
  LoreCodexUI_RealLogic1(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_1_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query1(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray1(origin, dir, maxDist=62){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic2(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_2_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query2(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray2(origin, dir, maxDist=64){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic3(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_3_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query3(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray3(origin, dir, maxDist=66){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic4(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_4_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query4(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray4(origin, dir, maxDist=68){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic5(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_5_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query5(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray5(origin, dir, maxDist=70){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic6(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_6_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query6(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray6(origin, dir, maxDist=72){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic7(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_7_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query7(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray7(origin, dir, maxDist=74){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic8(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_8_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query8(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray8(origin, dir, maxDist=76){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic9(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_9_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query9(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray9(origin, dir, maxDist=78){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic10(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_10_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query10(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray10(origin, dir, maxDist=80){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic11(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_11_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query11(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray11(origin, dir, maxDist=82){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic12(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_12_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query12(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray12(origin, dir, maxDist=84){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic13(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_13_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query13(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray13(origin, dir, maxDist=86){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic14(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_14_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query14(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray14(origin, dir, maxDist=88){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic15(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_15_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query15(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray15(origin, dir, maxDist=90){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic16(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_16_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query16(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray16(origin, dir, maxDist=92){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic17(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_17_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query17(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray17(origin, dir, maxDist=94){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic18(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_18_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query18(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray18(origin, dir, maxDist=96){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic19(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_19_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query19(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray19(origin, dir, maxDist=98){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic20(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_20_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query20(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray20(origin, dir, maxDist=100){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic21(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_21_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query21(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray21(origin, dir, maxDist=102){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic22(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_22_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query22(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray22(origin, dir, maxDist=104){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic23(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_23_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query23(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray23(origin, dir, maxDist=106){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic24(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_24_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query24(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray24(origin, dir, maxDist=108){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic25(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_25_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query25(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray25(origin, dir, maxDist=110){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic26(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_26_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query26(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray26(origin, dir, maxDist=112){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic27(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_27_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query27(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray27(origin, dir, maxDist=114){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic28(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_28_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query28(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray28(origin, dir, maxDist=116){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
  ...
  LoreCodexUI_RealLogic29(a,b,c){
    ...
    const contains=box.containsPoint(v1);
    this.cache.set('LoreCodexUI_29_'+Math.floor(this.time*12), len);
    return {len, dot, crossLen:cross.length(), contains, time:this.time};
  }
  LoreCodexUI_Query29(entities, dt){
    if(!entities||!entities.length) return 0;
    let acc=0, maxD=0;
  ...
  LoreCodexUI_Ray29(origin, dir, maxDist=118){
    const ray=new THREE.Ray(origin, dir.clone().normalize());
    const box=new THREE.Box3(
      new THREE.Vector3(origin.x-maxDist, origin.y-maxDist, origin.z-maxDist),
      new THREE.Vector3(origin.x+maxDist, origin.y+maxDist, origin.z+maxDist)
